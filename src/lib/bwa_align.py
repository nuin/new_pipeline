...
def run_bwa(
    ...
    if os.path.isfile(
        ...
        bwa_string = (
            f"{bwa} mem -t 16 -Y -R '{bam_header}' {reference} {' '.join([str(file) for file in fastq_files])} "
            f"| {samtools} view -bS -F 0 - | {samtools} sort -@ 16 -m 4G -o {datadir}/BAM/{sample_id}/BAM/{sample_id}.bam -"
        )
