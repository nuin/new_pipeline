...
def haplotype_caller(
    ...
    def _haplotype_caller():
        ...
        log_to_db(
            db,
            f"Starting HaplotypeCaller for sample {sample_id} with GATK3 version {gatk_version}",
            "INFO",
            "GATK3",
            sample_id,
            datadir.name,
        ...
        if output_vcf.exists():
            ...
            log_to_api(
                "GATK3 VCF file exists", "INFO", "GATK3", sample_id, datadir.name
            )
            log_to_db(
                db,
        ...
        console.print(
            Panel(
                f"[bold blue]Starting variant calling with GATK3 for {sample_id}[/bold blue]"
            )
        )
        log_to_api(
            "Start variant calling with GATK3", "INFO", "GATK3", sample_id, datadir.name
        )
        log_to_db(
            db,
            f"Starting variant calling with GATK3 for {sample_id}",
            "INFO",
            "GATK3",
            sample_id,
            datadir.name,
        ...
        for attempt in range(max_retries):
            ...
            log_to_db(
                db,
                f"GATK3 HaplotypeCaller started at {start_time} (Attempt {attempt + 1}/{max_retries})",
                "INFO",
                "GATK3",
                sample_id,
                datadir.name,
            ...
            with Progress(
                ...
                task = progress.add_task(
                    f"[cyan]Running GATK3 HaplotypeCaller for {sample_id}...",
                    total=None,
                )
                ...
                for line in process.stdout:
                    ...
                    log_to_db(
                        db, line.strip(), "DEBUG", "GATK3", sample_id, datadir.name
                    )
            ...
            log_to_db(
                db,
                f"GATK3 HaplotypeCaller finished at {end_time}. Duration: {duration}",
                "INFO",
                "GATK3",
                sample_id,
                datadir.name,
            ...
            if process.returncode == 0:
                ...
                log_to_db(
                    db,
                    f"GATK3 HaplotypeCaller failed. Retrying (Attempt {attempt + 2}/{max_retries})",
                    "WARNING",
                    "GATK3",
                    sample_id,
                    datadir.name,
        ...
        if output_vcf.exists():
            console.print(
                Panel(
                    f"[bold green]GATK3 variants determined for {sample_id}[/bold green]"
                )
            )
            log_to_api(
                "GATK3 variants determined", "INFO", "GATK3", sample_id, datadir.name
            )
            log_to_db(
                db,
                f"GATK3 variants determined successfully for {sample_id}",
                "INFO",
                "GATK3",
                sample_id,
                datadir.name,
