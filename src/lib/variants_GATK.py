...
def haplotype_caller(
    ...
    def _haplotype_caller():
        ...
        log_to_db(
            db,
            f"Starting HaplotypeCaller for sample {sample_id} with GATK version {gatk_version}",
            "INFO",
            "GATK",
            sample_id,
            datadir.name,
        ...
        if output_vcf.exists():
            console.print(
                Panel(f"[yellow]VCF file already exists for {sample_id}[/yellow]")
            )
            log_to_api("GATK VCF file exists", "INFO", "GATK", sample_id, datadir.name)
            log_to_db(
                db,
                f"VCF file already exists for {sample_id}",
                "INFO",
                "GATK",
                sample_id,
                datadir.name,
        ...
        console.print(
            Panel(
                f"[bold blue]Starting variant calling with GATK for {sample_id}[/bold blue]"
            )
        )
        log_to_api(
            "Start variant calling with GATK", "INFO", "GATK", sample_id, datadir.name
        )
        log_to_db(
            db,
            f"Starting variant calling with GATK for {sample_id}",
            "INFO",
            "GATK",
            sample_id,
            datadir.name,
        ...
        if dict_file.exists():
            console.print(
                Panel(
                    f"[green]Reference sequence dictionary already exists: {dict_file}[/green]"
                )
            )
            log_to_db(
                db,
                f"Using existing reference dictionary: {dict_file}",
                "INFO",
                "GATK",
                sample_id,
                datadir.name,
        ...
        gatk_command = (
            ...
            f"--annotation BaseQualityRankSumTest "
            f"--annotation MappingQualityRankSumTest "
            f"--annotation ReadPosRankSumTest "
        ...
        log_to_db(
            db, f"GATK command: {gatk_command}", "INFO", "GATK", sample_id, datadir.name
        )
        ...
        for attempt in range(max_retries):
            ...
            log_to_db(
                db,
                f"GATK HaplotypeCaller started at {start_time} (Attempt {attempt + 1}/{max_retries})",
                "INFO",
                "GATK",
                sample_id,
                datadir.name,
            ...
            with Progress(
                ...
                task = progress.add_task(
                    f"[cyan]Running GATK HaplotypeCaller for {sample_id}...", total=None
                )
                ...
                for line in process.stdout:
                    ...
                    log_to_db(
                        db, line.strip(), "DEBUG", "GATK", sample_id, datadir.name
                    )
            ...
            log_to_db(
                db,
                f"GATK HaplotypeCaller finished at {end_time}. Duration: {duration}",
                "INFO",
                "GATK",
                sample_id,
                datadir.name,
            ...
            if process.returncode == 0:
                ...
                log_to_db(
                    db,
                    f"GATK HaplotypeCaller failed. Retrying (Attempt {attempt + 2}/{max_retries})",
                    "WARNING",
                    "GATK",
                    sample_id,
                    datadir.name,
        ...
        if output_vcf.exists():
            console.print(
                Panel(
                    f"[bold green]GATK variants determined for {sample_id}[/bold green]"
                )
            )
            log_to_api(
                "GATK variants determined", "INFO", "GATK", sample_id, datadir.name
            )
            log_to_db(
                db,
                f"GATK variants determined successfully for {sample_id}",
                "INFO",
                "GATK",
                sample_id,
                datadir.name,
