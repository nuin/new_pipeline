...
def freebayes_caller(
    ...
    def _freebayes_caller():
        ...
        log_to_db(
            db,
            f"Starting Freebayes for sample {sample_id} with Freebayes version {freebayes_version}",
            "INFO",
            "Freebayes",
            sample_id,
            datadir.name,
        ...
        if output_vcf.exists():
            console.print(
                Panel(
                    f"[yellow]Freebayes VCF file already exists for {sample_id}[/yellow]"
                )
            )
            log_to_api(
            ...
            log_to_db(
                db,
                f"Freebayes VCF file already exists for {sample_id}",
                "INFO",
                "Freebayes",
                sample_id,
                datadir.name,
        ...
        console.print(
            Panel(
                f"[bold blue]Starting variant calling with Freebayes for {sample_id}[/bold blue]"
            )
        )
        log_to_api(
        ...
        log_to_db(
            db,
            f"Starting variant calling with Freebayes for {sample_id}",
            "INFO",
            "Freebayes",
            sample_id,
            datadir.name,
        ...
        console.print(
            Syntax(freebayes_command, "bash", theme="monokai", line_numbers=True)
        )
        log_to_db(
            db,
            f"Freebayes command: {freebayes_command}",
            "INFO",
            "Freebayes",
            sample_id,
            datadir.name,
        ...
        for attempt in range(max_retries):
            ...
            log_to_db(
                db,
                f"Freebayes started at {start_time} (Attempt {attempt + 1}/{max_retries})",
                "INFO",
                "Freebayes",
                sample_id,
                datadir.name,
            ...
            with Progress(
                ...
                task = progress.add_task(
                    f"[cyan]Running Freebayes for {sample_id}...", total=None
                )

                process = subprocess.Popen(
                    shlex.split(freebayes_command),
                ...
                for line in process.stdout:
                    ...
                    log_to_db(
                        db, line.strip(), "DEBUG", "Freebayes", sample_id, datadir.name
                    )
            ...
            log_to_db(
                db,
                f"Freebayes finished at {end_time}. Duration: {duration}",
                "INFO",
                "Freebayes",
                sample_id,
                datadir.name,
            ...
            if process.returncode == 0:
                ...
                log_to_db(
                    db,
                    f"Freebayes failed. Retrying (Attempt {attempt + 2}/{max_retries})",
                    "WARNING",
                    "Freebayes",
                    sample_id,
                    datadir.name,
                ...
                error_msg = (
                    f"Freebayes failed for {sample_id} after {max_retries} attempts"
                )
        ...
        if output_vcf.exists() and output_vcf.stat().st_size > 0:
            console.print(
                Panel(
                    f"[bold green]Freebayes variants determined for {sample_id}[/bold green]"
                )
            )
            log_to_api(
            ...
            log_to_db(
                db,
                f"Freebayes variants determined successfully for {sample_id}",
                "INFO",
                "Freebayes",
                sample_id,
                datadir.name,
            ...
            error_msg = (
                f"Freebayes completed but output VCF not found or empty for {sample_id}"
            )
            console.print(Panel(f"[bold red]{error_msg}[/bold red]"))
            log_to_api(error_msg, "ERROR", "Freebayes", sample_id, datadir.name)
...
def process_freebayes_vcf(
    datadir: Path, sample_id: str, reference: Path, db: Dict
) -> str:
    @timer_with_db_log(db)
    def _process_freebayes_vcf():
        ...
        try:
            console.print(
                Panel(
                    f"[bold blue]Processing Freebayes VCF for {sample_id}[/bold blue]"
                )
            )
            log_to_api(
                f"Start processing Freebayes VCF for {sample_id}",
                "INFO",
                "Freebayes",
                sample_id,
                datadir.name,
            ...
            sorted_variants = sorted(
                variants,
                key=lambda x: (
                    chrom_order.index(x[0])
                    if x[0] in chrom_order
                    else len(chrom_order),
                    int(x[1]),
                ),
            )
            ...
            console.print(
                Panel(
                    f"[bold green]Freebayes VCF processed and sorted successfully for {sample_id}[/bold green]"
                )
            )
            log_to_api(
                f"Freebayes VCF processed and sorted successfully for {sample_id}",
                "INFO",
                "Freebayes",
                sample_id,
                datadir.name,
            ...
            log_to_db(
                db,
                f"Processed VCF size: {input_vcf.stat().st_size} bytes",
                "INFO",
                "Freebayes",
                sample_id,
                datadir.name,
