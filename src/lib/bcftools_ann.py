...
def preprocess_vcf(
    ...
    """
    Function that calls bcftools to preprocess the merged VCF file.
    :param input_vcf:
    :param output_vcf:
    :param bcftools:
    :param db:
...
def annotate_merged(
    ...
    def _annotate_merged():
        ...
        for attempt in range(max_retries):
            ...
            log_to_db(
                db,
                f"bcftools csq started at {start_time} (Attempt {attempt + 1}/{max_retries})",
                "INFO",
                "bcftools_ann",
                sample_id,
                datadir.name,
            ...
            with Progress(
                ...
                task = progress.add_task(
                    f"[cyan]Running bcftools csq for {sample_id}...", total=None
                )
            ...
            log_to_db(
                db,
                f"bcftools csq finished at {end_time}. Duration: {duration}",
                "INFO",
                "bcftools_ann",
                sample_id,
                datadir.name,
            ...
            if process.returncode == 0:
                ...
                log_to_db(
                    db,
                    f"bcftools csq failed. Retrying (Attempt {attempt + 2}/{max_retries})",
                    "WARNING",
                    "bcftools_ann",
                    sample_id,
                    datadir.name,
                ...
                error_msg = (
                    f"bcftools csq failed for {sample_id} after {max_retries} attempts"
                )
        ...
        if output_vcf.exists():
            if transcript_list:
                ...
                try:
                    ...
                    with open(output_vcf, "r") as infile, open(
                        final_vcf, "w"
                    ) as outfile:
                        for line in infile:
                            if line.startswith("#"):
