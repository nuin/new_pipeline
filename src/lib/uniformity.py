...
def get_coverage_values(datadir, panel):
    ...
    for nucl_file in glob.glob(f"{datadir}/BAM/*/Metrics/*.nucl.out"):
        ...
        uniformity_file.write(str(values['cov_02'].value_counts().loc[True] / panel_length) + '\t')
        uniformity_file.write(str(values['cov_05'].value_counts().loc[True] / panel_length) + '\t')
        uniformity_file.write(str(values['cov_m'].value_counts().loc[True] / panel_length) + '\t')
