...
def vep_annotate(
    ...
    def _annotate_merged():
        ...
        log_to_db(
            db,
            f"Starting VEP annotation for sample {sample_id} with VEP version {vep_version}",
            "INFO",
            "vep_ann",
            sample_id,
            datadir.name,
        ...
        if output_vcf.exists():
            console.print(
                Panel(
                    f"[yellow]Annotated VCF file already exists for {sample_id}[/yellow]"
                )
            )
            log_to_api(
                "Annotated VCF file exists", "INFO", "vep_ann", sample_id, datadir.name
            )
            log_to_db(
                db,
                f"Annotated VCF file already exists for {sample_id}",
                "INFO",
                "vep_ann",
                sample_id,
                datadir.name,
        ...
        console.print(
            Panel(f"[bold blue]Starting VEP annotation for {sample_id}[/bold blue]")
        )
        log_to_api("Start VEP annotation", "INFO", "vep_ann", sample_id, datadir.name)
        log_to_db(
            db,
            f"Starting VEP annotation for {sample_id}",
            "INFO",
            "vep_ann",
            sample_id,
            datadir.name,
        ...
        subprocess.run(
            ["sudo", "chown", "systemd-coredump:input", str(vep_input)], check=True
        )
        ...
        log_to_db(
            db, f"VEP command: {vep_cmd}", "INFO", "vep_ann", sample_id, datadir.name
        )
        ...
        for attempt in range(max_retries):
            ...
            log_to_db(
                db,
                f"VEP started at {start_time} (Attempt {attempt + 1}/{max_retries})",
                "INFO",
                "vep_ann",
                sample_id,
                datadir.name,
            ...
            with Progress(
                ...
                task = progress.add_task(
                    f"[cyan]Running VEP for {sample_id}...", total=None
                )
                ...
                for line in stdout.splitlines():
                    ...
                    log_to_db(
                        db, line.strip(), "DEBUG", "vep_ann", sample_id, datadir.name
                    )
            ...
            log_to_db(
                db,
                f"VEP finished at {end_time}. Duration: {duration}",
                "INFO",
                "vep_ann",
                sample_id,
                datadir.name,
            ...
            if process.returncode == 0:
                ...
                log_to_db(
                    db,
                    f"VEP failed. Retrying (Attempt {attempt + 2}/{max_retries})",
                    "WARNING",
                    "vep_ann",
                    sample_id,
                    datadir.name,
        ...
        if vep_output.exists():
            ...
            subprocess.run(
                ["sudo", "chown", f"{os.getuid()}:{os.getgid()}", str(vep_output)],
                check=True,
            )
            subprocess.run(["sudo", "cp", str(vep_output), str(output_vcf)], check=True)
            subprocess.run(
                ["sudo", "rm", str(vep_input)], check=True
            )  # Remove the temporary input file
            subprocess.run(
                ["sudo", "rm", str(vep_output)], check=True
            )  # Remove the temporary output file
            console.print(
                Panel(
                    f"[bold green]VEP annotation completed for {sample_id}[/bold green]"
                )
            )
            log_to_api(
                "VEP annotation completed", "INFO", "vep_ann", sample_id, datadir.name
            )
            log_to_db(
                db,
                f"VEP annotation completed successfully for {sample_id}",
                "INFO",
                "vep_ann",
                sample_id,
                datadir.name,
